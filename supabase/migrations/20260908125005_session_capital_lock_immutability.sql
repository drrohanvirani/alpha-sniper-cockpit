-- PREPARED ONLY. Based on owner-supplied production definitions.
-- Only the two existing functions below are replaced; no ACL/schema changes.
-- Existing identity: trade_date, upper(ticker), campaign (NULL-safe).
-- Financial snapshots are frozen, not retry identity. No existing-row UPDATE.
-- Requires both functions to exist with their verified backend-only ACLs.
-- Apply as one transaction after separate review/approval, never replay baseline.sql.
...
begin
  ...
  -- Shared namespace 1095520332 (0x414C504C, ALPL); date = days since 2000-01-01.
  -- Always acquire in this order: transaction advisory lock, then session row lock.
  perform pg_catalog.pg_advisory_xact_lock(1095520332, p_trade_date - date '2000-01-01');
  select * into v_row from public.alpha_session_capital_lock
  where trade_date=p_trade_date for update;
  if found then
    if upper(v_row.capital_owner_ticker) is distinct from upper(p_capital_owner_ticker)
       or v_row.capital_owner_campaign_id is distinct from p_campaign_id then
      raise exception 'SESSION_CAPITAL_LOCK_CONFLICT' using errcode='P0001';
    end if;
...
declare
  ...
  v_volume_rule jsonb;
  v_kill_rule jsonb;
  v_existing public.alpha_session_capital_lock%rowtype;
  v_exists boolean;
  v_input_error text;
begin
  ...
  -- Same two-part namespace/date key as alpha_lock_session_capital().
  perform pg_catalog.pg_advisory_xact_lock(1095520332, v_day - date '2000-01-01');
  select * into v_existing from public.alpha_session_capital_lock
  where trade_date=v_day for update;
  v_exists:=found;

  select current_truth->'capital_plan' into v_plan
  from public.brain_state where state_key='ALPHA-OS-CANONICAL';
  if v_exists then
    -- Resolve current identity without writes. First-create interpretation below
    -- remains unchanged. Bad current evidence cannot overwrite a stored session.
    <<retry_input>>
    begin
      if v_plan is null or jsonb_typeof(v_plan)<>'object'
         or upper(coalesce(v_plan->>'status',''))<>'LOCKED' then
        v_input_error:='NO_LOCKED_CAPITAL_PLAN'; exit retry_input;
      end if;
      v_locked_at:=nullif(v_plan->>'locked_at','')::timestamptz;
      v_expires:=nullif(v_plan->>'expires_at','')::timestamptz;
      if v_locked_at is null or (v_locked_at at time zone 'Asia/Kolkata')::date<>v_day
         or v_expires is null or v_expires<=v_now then
        v_input_error:='LOCKED_PLAN_NOT_CURRENT_SESSION'; exit retry_input;
      end if;
      if jsonb_typeof(coalesce(v_plan->'items','[]'::jsonb))<>'array' then
        v_input_error:='INVALID_PLAN_ITEMS'; exit retry_input;
      end if;
      ...
      if v_cash is null or v_cash<0 or v_nav is null or v_nav<=0 then
        v_input_error:='INVALID_CASH_OR_NAV'; exit retry_input;
      end if;
      v_owner:=upper(coalesce(v_plan->>'capital_owner','CASH'));
      if v_owner='CASH' or jsonb_array_length(coalesce(v_plan->'items','[]'::jsonb))=0 then
        v_owner:='CASH';
        v_campaign_id:=coalesce(v_plan->>'lock_id','CAPITAL-OWNER-'||to_char(v_day,'YYYYMMDD'));
      else
        ...
        if v_item is null or btrim(v_owner)='' then
          v_input_error:='OWNER_ITEM_MISSING'; exit retry_input;
        end if;
    ...
    exception
      when invalid_text_representation or numeric_value_out_of_range
        ...
        v_input_error:='MALFORMED_CURRENT_INPUT';
    end retry_input;

    if v_input_error is not null then
      return jsonb_build_object('ok',false,
        'status','SESSION_LOCK_PRESERVED_CURRENT_INPUT_INVALID','reason',v_input_error,
        'for_date',v_day,'existing_status',v_existing.status,
        'owner',v_existing.capital_owner_ticker,'campaign_id',v_existing.capital_owner_campaign_id,
        'session',to_jsonb(v_existing));
    end if;
    if upper(v_existing.capital_owner_ticker) is distinct from v_owner
       or v_existing.capital_owner_campaign_id is distinct from v_campaign_id then
      return jsonb_build_object('ok',false,'status','SESSION_CAPITAL_LOCK_CONFLICT',
        'for_date',v_day,'existing_status',v_existing.status,
        'owner',v_existing.capital_owner_ticker,'campaign_id',v_existing.capital_owner_campaign_id,
        'session',to_jsonb(v_existing));
    end if;
    return jsonb_build_object('ok',true,'status',v_existing.status,'idempotent_retry',true,
      'for_date',v_day,'owner',v_existing.capital_owner_ticker,
      'campaign_id',v_existing.capital_owner_campaign_id,'session',to_jsonb(v_existing));
  end if;
